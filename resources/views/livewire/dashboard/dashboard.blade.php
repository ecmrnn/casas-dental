                                ...
                                @foreach ($scheduled as $record)
                                    <tr key="{{ $record->id }}">
                                        ...
                                        <td class="py-2 px-5 border-y border-r border-gray-200 rounded-e-lg text-right">
                                            <button wire:click="viewRecord({{ $record->id }})" class="px-5 py-2 rounded-lg border border-gray-200 hover:bg-gray-50">
                                                View Record
                                            </button>
                                        </td>
                                    </tr>
                    ...
                    <div class="md:hidden space-y-2">
                        @foreach ($scheduled as $record)
                            <div class="rounded-lg bg-white border border-gray-200 flex justify-between relative">
                                <button wire:click="viewRecord({{ $record->id }})" class="w-full text-left flex items-center pr-4 flex-grow">
                                    @if ($record->status !== 'completed')
                                        @if ($record->status == 'scheduled')
                                            @if ($record->schedule_date . " " . $record->schedule_time > date('Y-m-d H:i:s'))
                                ...
                                <button wire:click="" class="p-2 m-2 rounded-full grid place-items-center border border-transparent hover:border-gray-200 hover:bg-gray-50 transition-all ease-in-out duration-200">
